...
Discord Bot は現時点では `/help` と `/add text:` を実装しています。`/add` は自然文から予定候補を作り、Embed と「登録する」「キャンセル」ボタンで確認します。Google Calendar への予定登録は Step 3 で実装予定です。
...
- `GOOGLE_CLIENT_ID`: Google Calendar 連携用。Step 1 では未使用です。
- `GOOGLE_CLIENT_SECRET`: Google Calendar 連携用。Step 1 では未使用です。
- `GOOGLE_REFRESH_TOKEN`: Google Calendar 連携用。Step 1 では未使用です。
- `GOOGLE_CALENDAR_ID`: Google Calendar の保存先。Step 1 では未使用です。
